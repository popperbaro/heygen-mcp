...
class HeyGenApiClient:
    ...
    async def list_avatar_groups(
        ...
        async def api_call():
            endpoint = f"avatar_group.list?include_public={'true' if include_public else 'false'}"
...
# Create MCP server and API client
...
# TODO: paginate all voices
@mcp.tool(
    name="get_voices",
    description="Retrieves a list of available voices from the HeyGen API. Results truncated to first 100 voices.",
)
...
@mcp.tool(
    ...
    description="Retrieves a list of HeyGen avatar groups.",
)
async def get_avatar_groups(include_public: bool = False) -> MCPAvatarGroupResponse:
    """List available HeyGen avatar groups via HeyGen API v2/avatar_group.list endpoint."""
    try:
        client = await get_api_client()
...
@mcp.tool(
    ...
    description="Retrieves the status of a video generated via the HeyGen API.",
)
